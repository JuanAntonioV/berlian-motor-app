...
const baseGoodsReceiptSchema = createInsertSchema(goodsReceipts, {
  invoiceNumber: (schema) =>
    schema.invoiceNumber.min(1, 'ID tidak boleh kosong'),
...
export const goodsReceiptItems = pgTable('goods_receipt_items', {
  id: serial('id').primaryKey(),
  goodsReceiptId: bigint('goods_receipt_id', { mode: 'bigint' })
    .notNull()
...
export const stockTransfers = pgTable(
  'stock_transfers',
  {
    ...
    toStoreId: integer('to_store_id')
      .notNull()
      .references(() => stores.id, { onDelete: 'cascade' }),
    fromShelfId: integer('from_shelf_id')
      .notNull()
      .references(() => shelfs.id, { onDelete: 'cascade' }),
    toShelfId: integer('to_shelf_id')
      .notNull()
      .references(() => shelfs.id, { onDelete: 'cascade' }),
    attachment: varchar('attachment'),
...
export const stockTransferItems = pgTable('stock_transfer_items', {
  id: serial('id').primaryKey(),
  stockTransferId: bigint('stock_transfer_id', { mode: 'bigint' })
    .notNull()
...
export const productExpenditures = pgTable(
  'product_expenditures',
  {
    id: bigserial('id', { mode: 'bigint' }),
    invoiceNumber: varchar('invoice_number', { length: 36 }).notNull().unique(),
    userId: bigint('id', { mode: 'bigint' })
...
export const productExpenditureItems = pgTable('product_expenditure_items', {
  id: serial('id').primaryKey(),
  productExpenditureId: bigint('product_expenditure_id', { mode: 'bigint' })
    .notNull()
...
export const stockAdjustmentItems = pgTable('stock_adjustment_items', {
  id: serial('id').primaryKey(),
  stockAdjustmentId: bigint('stock_adjustment_id', { mode: 'bigint' })
    .notNull()
